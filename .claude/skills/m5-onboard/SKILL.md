...
3. **Fetch firmware** (`fetch_firmware.py`) — query the M5Burner manifest API and download the appropriate UIFlow 2.0 binary into the system temp dir. Most variants pull the newest stable release; the **Cardputer-Adv is pinned to v2.4.8** because v2.4.9+ breaks the keyboard (see "Critical gotchas"). Cached between runs — safe to clear the cache anytime, it just re-downloads.
...
- **Cardputer-Adv firmware is pinned to UIFlow2 v2.4.8 — do NOT flash v2.4.9+.** `fetch_firmware.py` pins the `cardputer-adv` variant to `v2.4.8` (via `version_pin`); every other variant still tracks latest. UIFlow2 **v2.4.9 reworked the Adv keyboard subsystem**: the keypad moved fully behind the TCA8418 I2C controller (`hardware.Keyboard`), and `hardware.MatrixKeyboard` — the driver the buddy bundle uses for input — **stops reading keys on the Adv entirely.** The failure is nasty because everything *looks* fine: the flash verifies, the device boots into the launcher, the animation plays, BLE even works — but the arrow keys and Enter are dead, so the user can never navigate to launch Claude Buddy. Under the hood the TCA8418 IRQ fires on each keypress but its I2C read throws `OSError: 259` (`ESP_ERR_INVALID_STATE`). v2.4.8 is the last build where `MatrixKeyboard.get_key()` returns the arrow cluster (`;` `,` `.` `/`) on the Adv. If you ever bump the pin, re-test the keyboard **on real Adv hardware, end-to-end** (flash → install bundle → confirm arrows move the launcher highlight) before committing — a green flash and a working BLE link are not enough. The proper long-term fix is to port the bundle's input layer from `MatrixKeyboard` to `hardware.Keyboard`; until then, keep the pin.
